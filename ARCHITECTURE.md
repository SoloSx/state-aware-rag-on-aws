...
論文「Reasoning with Memory: Adaptive Information Management for Retrieval-Augmented Generation」(Man et al., Amazon AI) を **AWS 上で実際に動かす**ための構成。
...
- 学習は **Path-Outcome Dual Reward** (GRPO)。ただし論文 Table 3 では **「Claude をそのまま Extractor に使う」構成が最高性能** → **学習なしで Amazon Bedrock 上に再現可能**。
...
| Judge = Claude 3.7 Sonnet | Amazon Bedrock Claude Sonnet 4.5 (東京リージョンに3.7が無いため4.5に統一) |
| Retriever = Qwen3-Embedding-4B + FAISS | Amazon Bedrock Knowledge Bases (Amazon S3 Vectors + Amazon Titan Embeddings v2) |
| LiteLLM プロキシ / ロードバランス | Amazon Bedrock 側でマネージド (adaptive retry) |
| Wikipedia 2023 dump | Amazon S3 に格納し Amazon Bedrock Knowledge Bases のデータソースに |
...
```
   ...
   └──────────┘ ◄─────────────── │        │  ┌─────────── Working Memory ───┐ │
                  ...
                  │   ┌──────────────┐        ┌─────────────────────────┐   │
                  │   │  Amazon S3   │ ─sync─►│  Amazon S3 Vectors      │   │
                  │   │  (corpus)    │        │  (vector index, 1024)   │   │
                  │   └──────────────┘        └─────────────────────────┘   │
                  └──────────────────────────────────────────────────────────┘
```
...
2. Generator: 検索クエリ生成 → Retriever (Amazon Bedrock Knowledge Bases) で文書 D_i 取得
...
|---|---|---|---|
| **ローカル検証** | FAISS (`SAR_RETRIEVER_BACKEND=faiss`) | Amazon Bedrock | ローカル Python |
| **本番 (推奨)** | Amazon Bedrock Knowledge Bases (`bedrock_kb`) | Amazon Bedrock | Lambda / ECS Fargate |
| **学習版 Extractor** | Amazon Bedrock Knowledge Bases | Generator=Amazon Bedrock, Extractor=SageMaker エンドポイント | ECS Fargate |
...
- ベクトルストアは Amazon S3 Vectors を採用。Amazon OpenSearch Serverless と異なり常時の計算課金が発生しないため、低 QPS の検証用途に向く。
